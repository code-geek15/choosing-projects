<?php 
	...
	// connect to database
	$db = mysqli_connect('localhost', 'root', '', 'geekschat');
		...
		if (count($errors) == 0)
		{
			...
			$query = "SELECT * FROM users WHERE username = '$username'";
   			 ...
   			 if (mysqli_num_rows($result) > 0) {
       			 // Username already exists, display an alert message and ask for input again
       			 echo "<script>alert('Username already exists. Please choose a different username.');</script>";
        // You might want to redirect the user to the registration page again or show the registration form.
			} else {
				// Username is unique, proceed with user registration
				$query = "INSERT INTO users (username, email, user_type, password) 
						VALUES('$username', '$email', 'chat_user', '$password')";
				mysqli_query($db, $query);

				// Get the ID of the created user
				$logged_in_user_id = mysqli_insert_id($db);

				$_SESSION['user'] = getUserById($logged_in_user_id); // Put the logged-in user in the session
	...
	function login()
	{
		...
		if (count($errors) == 0)
		{
			...
			{ // user found
				...
				if ($logged_in_user['user_type'] == 'chat_user') {
	...
	function isAdmin()
	{
		if (isset($_SESSION['user']) && $_SESSION['user']['user_type'] == 'chat_user' )
		{
			return true;
		}else
		{
